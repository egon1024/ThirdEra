...
- **Apply damage/heal** — Apply the total from a damage or healing roll (in chat or from the sheet) to selected token(s) or a chosen actor. **Phases 2–4** are implemented: Phase 2 (entry points); Phase 3 (temp HP, nonlethal, healing reduces nonlethal); Phase 4 (combined “Attack & Damage” roll posts one message, Apply uses damage total; `openWithOptions` passes `nonlethal`). Chat “Apply” button and context menu; sheet “Apply damage/healing” and “Apply to this token”; weapon row “Attack & Damage” button; macro `game.thirdera.applyDamageHealing.openDialog()` or `openWithOptions({ targetActors, amount, mode, nonlethal })`. See [Testing Apply damage/healing (Phase 2)](development.md#testing-apply-damagehealing-phase-2), [Phase 3](development.md#testing-apply-damagehealing-phase-3), and [Phase 4](development.md#testing-apply-damagehealing-phase-4) for manual test steps.
