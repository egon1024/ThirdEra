...
- **Linting** — **ESLint** runs in **Validate** (`lint` job) and locally (`npm run lint`, `make lint`) on **`test/**/*.mjs`**, **`module/**/*.mjs`**, **`thirdera.mjs`**, **`scripts/**/*.mjs`**, and **`vitest.config.mjs`**. Optional follow-up: tighten rules, Foundry global coverage, or split jobs. Commands and policy: [Development — Automated unit tests](development.md#automated-unit-tests).
