...
2. **Item method `getModifierChanges(actor)`.** On any item type, implement `getModifierChanges(actor)` returning `{ applies: boolean, changes: Array<{ key: string, value: number, label?: string }> }`. The built-in item provider in the registry iterates `actor.items` and, when this method exists, calls it; when `applies === true`, it merges the returned `changes` with `label = item.name`. No change to the aggregator is required. *(Note: the stock `itemsModifierProvider` today reads `system.changes` directly for feat, race, and equipped items; `getModifierChanges` is the documented extension hook for custom item types.)* Items can use optional `system.changes` (same shape) and rely on the default item provider’s type-specific “applies” rules (feat and race: always when owned; equipment/armor/weapon: when equipped).
...
- **Items:** Race, feat, armor, weapon, and equipment sheets include **Capability grants** for sense rows stored on the item. Feats apply while owned; gear applies when equipped under the same rules as mechanical modifiers. Optional raw `system.cgsGrants.grants` (advanced authoring) overrides the simple sense list when non-empty. Conditions use **`system.cgsGrants.grants`** for structured effects such as sense suppression.
