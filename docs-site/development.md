...
**Canonical templates + per-owned overrides (optional):** Feats, class features, creature features, armor, equipment, and weapons may store **`system.cgsGrantOverrides`** (partial `{ grants, senses }` in the same shapes as base Capability grants) merged over a **resolved template** — the compendium or world **Item** referenced by Foundry’s normal **`sourceId`** lineage, or **`system.cgsTemplateUuid`** when authors need an explicit document UUID. Merge semantics and resolution rules are implemented in **`module/logic/cgs-grant-template-merge.mjs`** and applied at contribution time from **`module/logic/cgs-owned-item-grants.mjs`**. Item sheets expose **CGS parameters and overrides** under Details: a **status line** shows which library item drives the merge, **Choose library item** opens a searchable picker, and a **drop zone** accepts a compatible Item from the sidebar (GMs do not need to paste document UUIDs on the sheet). **Reset CGS overrides** clears overrides and the explicit template UUID. When a **world** template item is saved, embedded copies on actors receive template updates but **preserve** per-copy overrides. A GM-only world setting (**`cgsGrantOverridesWorldMigrationRevision`**) records an idempotent migration gate on first load after upgrade (including **legacy creature-feature embeds** rewritten via **`module/logic/cgs-creature-feature-consolidation.mjs`**). For bulk **`packs/monsters/*.json`** rewrites of those embeds, run **`node scripts/apply-cgs-template-manifest.mjs`** (dry-run by default; **`--write`** to apply). The companion **`scripts/cgs-template-consolidation-manifest.json`** documents that workflow; rules live in the consolidation module.
