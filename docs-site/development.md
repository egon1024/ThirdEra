...
- **Type-based mechanics:** Turning/rebuking, favored enemy, spells that only target a creature type (e.g. humanoid-only), smite-style riders keyed to type, and similar **rules** are **not** implemented as automated core resolution—authors and GMs apply the rules at the table. **Gear** may use **`mechanicalCreatureGateUuids`** so modifiers and CGS from that item apply only when **`getEffectiveCreatureTypes`** matches; that is not full “item proficiency” or spell targeting automation. When further features are added to core, they must use **`getEffectiveCreatureTypes`** (not **Details** alone). See maintainer **`cgs-phased-implementation.md`** (effective-type consumers backlog).
