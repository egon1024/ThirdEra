...
- **Type-based mechanics:** Turning/rebuking, favored enemy, spells that only target a creature type (e.g. humanoid-only), equipment usable only by certain types, smite-style riders keyed to type, and similar **rules** are **not** implemented as automated core resolution—authors and GMs apply the rules at the table. When such features are added to core, they must use **`getEffectiveCreatureTypes`** (not **Details** alone). See maintainer **`cgs-phased-implementation.md`** (effective-type consumers backlog).
...
- **Typed-defense catalogs as compendium Items** (optional extensibility beyond CONFIG): **not** implemented; vocabulary remains CONFIG-backed unless/until that track ships.
