...
3. **`make lint`** or **`npm run lint`** — runs **ESLint** on `test/**/*.mjs`, `module/**/*.mjs`, `thirdera.mjs`, `scripts/**/*.mjs`, and `vitest.config.mjs` ([`eslint.config.js`](../eslint.config.js)); same scope as the **Validate** workflow **`lint`** job.

**Day-to-day:** After dependencies are installed, **`make test`** or **`npm test`** is enough for quick iteration. Use `npm run test:watch` for interactive re-runs while editing (not used in CI). Before finishing work that touches scoped backend logic, also run **`make test-coverage`** so **minimum coverage** (see below) is satisfied—the same command CI runs. After editing tests or system JavaScript under `module/` (or the other linted paths), run **`make lint`** (or `npm run lint`) before pushing.
