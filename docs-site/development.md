...
- **Usage:** Call `getActiveModifiers(actor)` once early in character `prepareDerivedData` (after base + racial ability effective); apply ability deltas and set mod; then use the same modifier bag for AC, speed, saves, and attack. Extenders can push additional provider functions to the registry so new item types or effects participate without editing the aggregator.
