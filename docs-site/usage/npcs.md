...
- **Attributes** - **Details** subtab: Challenge Rating, alignment, size, **Creature Type** and **Subtypes** (drag type/subtype items from compendiums). **Scores & Saves** subtab: ability scores and saving throws. **Feats** and **Class Features** subtabs list embedded feat and feature items (and any class-granted entries when the NPC has class levels), similar to the character sheet; drag feats or features from compendiums onto the sheet to add them.
