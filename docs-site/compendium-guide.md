...
- **Canonical templates + per-embed overrides:** Parametrized qualities (e.g. **darkvision** distances, **blindsight** / **blindsense** / **tremorsense** ranges, **DR /—** and **DR /magic**, **energy resistance** by type, **fast healing**) ship as **one compendium row per family** with stable **`system.key`** (e.g. `creatureDarkvision`, `creatureEnergyResistance`) and variance on **actor-owned copies** under **`system.cgsGrantOverrides`**, merged at runtime per [Development](development.md). Maintainer transforms for `packs/monsters/*.json` live in **`module/logic/cgs-creature-feature-consolidation.mjs`**; apply with **`node scripts/apply-cgs-template-manifest.mjs`** (dry-run by default, **`--write`** to save). **World upgrade:** when **`cgsGrantOverridesWorldMigrationRevision`** advances, the GM client rewrites **legacy embedded keys** on actors to the same canonical shape. **Bundled creature-features pack refresh:** bump **`BUNDLED_COMPENDIUM_SYNC_MIGRATION_REVISION`** in `module/logic/compendium-bundled-json-sync.mjs` when `FILE_MAPPINGS` or pack JSON changes so worlds re-import that compendium.
- **Curated seed set (examples):** initiative and attack or save bonuses (numeric **Mechanical effects**), Hide skill bonus, **darkvision** / **blindsight** / **blindsense** / **tremorsense** (canonical templates + per-monster overrides for distance), low-light, scent, poison immunity, **sleep / paralysis / stunning / disease** immunities, **energy resistance** (canonical `creatureEnergyResistance` + overrides per type/amount), **DR** (`creatureDamageReductionNone`, `creatureDamageReductionMagic` + overrides), **Multiattack** (`creatureMultiattack`) with empty **Mechanical effects** and prose that explains SRD natural-attack behavior vs the NPC sheet’s **Natural attacks** list and optional **preset attack bonus**, plus prose-first special attacks (**improved grab**, **constrict**, **swallow whole**, **rend**, **pounce**, **rake**, **trample**, **blood drain**) and **fast healing** (`creatureFastHealing`). For **feat-identical** bonuses on monsters, prefer embedded **`feat`** items per [Feat vs creature feature (authoring)](#monster-npc-feat-vs-creature-feature). Pack descriptions use **What it does (SRD)** and **ThirdEra** sections with **player-facing sheet labels**, not internal storage field names.
